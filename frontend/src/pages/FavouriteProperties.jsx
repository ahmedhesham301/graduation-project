...
export default function FavouriteProperties({ onNavigate, theme, toggleTheme, isLoggedIn, currentUser }) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to remove from favorites?")) return;

    try {
      await api.delete(`/favorites/${id}`);
      setAllProperties(prev => prev.filter(p => p.property_id !== id));
    } catch (err) {
      console.log(err);
    }
  };
            ...
            <div className="fp-card" key={p.property_id} onClick={() => {onNavigate("propertyDetails", { id: p.property_id , from: "favourite" });}} style={{ cursor: "pointer" }}>
              ...
              <div className="fp-card-info">
                <div className="fp-card-top">
                  <div>
                    <div className="fp-card-price">EGP {Number(p.price).toLocaleString()}</div>
                    ...
                    <div className="fp-fav-btn active">
                ...
                <p className="fp-card-title">{p.description}</p>
                ...
                <div className="fp-card-actions">
                  ...
                  <button className="fp-delete-btn" onClick={(e) => { e.stopPropagation(); handleDelete(p.property_id);}}>
